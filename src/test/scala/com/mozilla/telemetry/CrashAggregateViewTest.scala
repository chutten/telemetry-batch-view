...
class CrashAggregateViewTest extends FlatSpec with Matchers with BeforeAndAfterAll {
  ...
  def fixture = {
    ...
    def createPing(dimensions: Map[String, Any]): Map[String, Any] = {
      ...
      Map(
        ...
        "profile" -> compact(render(profile))
      )
    }
  ...
  "dimensions" must "be converted correctly" in {
    ...
    val profile_ages = validActivityDates
      .map(date => Days.daysBetween(date, new DateTime(0).plusDays(dimensionValues("profile_created")(0).asInstanceOf[String].toInt)).getDays().toString)
    for (row <- fixture.records.select("dimensions").collect()) {
      ...
      assert(profile_ages                         contains dimensions.getOrElse("profile_created", null))
    }
  }
